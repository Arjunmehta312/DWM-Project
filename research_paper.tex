...
\href{https://colab.research.google.com/drive/1mSyvYcl-5a8aZPziLIpSWPAy2do65HaP?usp=sharing}{[Python Code]} \quad
\href{https://drive.google.com/file/d/1RW1vYYOeDhcuN6i1VPSBoXwPqQoqoaoL/view?usp=sharing}{[Dataset]}}

\author{
\IEEEauthorblockN{Arjun Mehta, Dakshita Galhotra, Jal Bafana}
\IEEEauthorblockA{B. Tech CSE (Cybersecurity), 2nd Year\\
Mukesh Patel School of Technology Management and Engineering\\
Data Warehousing and Mining - Semester 4 Project\\
Roll Numbers: K036, K021, K005 \quad SAP ID: 70102300018, 70102300055, 70102300054}
}
...
\begin{IEEEkeywords}
AI coding tools, software development, developer productivity, data mining, k-means clustering, ChatGPT, GitHub Copilot, trust in AI
\end{IEEEkeywords}
...
% Create a page break to ensure the figure appears on a clean page
\begin{figure}[H]
    \centering
    % Add extra vertical space before the figure
    \vspace{0.5cm}
    % Reduce figure width to make the image smaller
    \includegraphics[width=0.5\textwidth, keepaspectratio]{final_research/software_request_categories.png}
    \caption{Categories of requested autonomous software}
    \label{fig:software_requests}
    % Add extra space after the figure
    \vspace{0.5cm}
\end{figure}
...
\section{K-means Clustering Analysis of Open-Ended Responses}
...
\begin{enumerate}
    \item Desired Improvements for AI coding tools
    \item Additional Comments about AI coding experiences
    \item Autonomous Software Requests (desired future applications)
\end{enumerate}

\subsection{Methodology}
For each open-ended question, we performed the following steps:
\begin{itemize}
    \item Text preprocessing (tokenization, removal of stopwords)
    \item Feature extraction using TF-IDF vectorization
    \item Silhouette score analysis to determine optimal cluster counts
    \item K-means clustering with the optimal number of clusters
    \item Visualization and interpretation of resulting clusters
\end{itemize}
...
\subsection{Desired Improvements Clustering}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{analysis/plots/cluster_scatter_desired_improvements.png}
    \caption{PCA visualization of clusters for Desired Improvements}
    \label{fig:desired_improvements_clusters}
\end{figure}

The most significant clusters included:
\begin{itemize}
    \item \textbf{Explanation-Focused (23.7\%):} Improvements in explanations, code clarity, and domain-specific understanding, primarily requested by students and early-career developers.
    \item \textbf{Traditional Support (15.5\%):} Traditional programming approaches and development methodologies, predominantly requested by experienced developers skeptical of AI.
    \item \textbf{Research Integration (13.4\%):} Better context understanding, citation mechanisms, and research workflow integration, primarily from researchers.
    \item \textbf{Advanced Features (12.4\%):} Advanced machine learning models and collaboration tools, predominantly from data scientists.
    \item \textbf{Security Features (8.2\%):} Enhanced security features and analysis capabilities, primarily from freelancers and client-facing roles.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{analysis/plots/desired_improvements_cluster_3_wordcloud.png}
        \caption{Explanation-Focused Cluster}
        \label{fig:sub-explanation}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{analysis/plots/desired_improvements_cluster_6_wordcloud.png}
        \caption{Traditional Support Cluster}
        \label{fig:sub-traditional}
    \end{subfigure}
    \caption{Word clouds visualizing key terms in the two largest clusters}
    \label{fig:improvements_wordclouds}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{analysis/plots/role_vs_desired_improvements_cluster_heatmap.png}
    \caption{Relationship between Professional Role and Desired Improvements Clusters}
    \label{fig:role_improvements_heatmap}
\end{figure}

\subsection{Additional Comments Clustering}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{analysis/plots/cluster_scatter_additional_comments.png}
    \caption{PCA visualization of clusters for Additional Comments}
    \label{fig:additional_comments_clusters}
\end{figure}

The most significant clusters included:
\begin{itemize}
    \item \textbf{AI as Supplement (16.5\%):} Comments focusing on AI as complementary to human skills rather than a replacement, with mentions of enabling previously difficult tasks.
    \item \textbf{Academic Integrity (13.4\%):} Maintaining research standards and academic boundaries while leveraging AI, primarily from researchers.
    \item \textbf{Workflow Transformation (12.4\%):} Emphasis on how AI is transforming data science and professional workflows, predominantly from data scientists.
    \item \textbf{Balancing Act (11.3\%):} Comments about finding the right balance between AI assistance and preserving personal creativity and skills.
    \item \textbf{Educational Tool (12.4\%):} Focus on AI's role in learning and navigating programming concepts, primarily from students.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{analysis/plots/additional_comments_cluster_5_wordcloud.png}
        \caption{Educational Tool Cluster}
        \label{fig:sub-educational}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{analysis/plots/additional_comments_cluster_7_wordcloud.png}
        \caption{Workflow Transformation Cluster}
        \label{fig:sub-workflow}
    \end{subfigure}
    \caption{Word clouds visualizing key terms in two representative comment clusters}
    \label{fig:comments_wordclouds}
\end{figure}
...
\subsection{Autonomous Software Request Clustering}
The clustering of requested autonomous software capabilities revealed distinct categories of desired future applications.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{analysis/plots/cluster_scatter_autonomous_software_request.png}
    \caption{PCA visualization of clusters for Autonomous Software Requests}
    \label{fig:software_request_clusters}
\end{figure}

The most significant clusters included:
\begin{itemize}
    \item \textbf{Learning Tools (28.9\%):} Personalized, adaptive learning platforms focusing on coding education and skill development.
    \item \textbf{Research Platforms (10.3\%):} Comprehensive research collaboration tools with scientific data management capabilities.
    \item \textbf{Project Management (11.3\%):} Secure freelance and project management systems with enhanced features for client work.
    \item \textbf{Data Analysis (10.3\%):} Comprehensive data analysis, visualization, and automated processing tools.
    \item \textbf{Intelligent Design Assistants (7.2\%):} Tools focusing on software architecture and intelligent design assistance.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{analysis/plots/autonomous_software_request_cluster_9_wordcloud.png}
        \caption{Visualization \& Algorithm Tools}
        \label{fig:sub-algorithm}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{analysis/plots/autonomous_software_request_cluster_8_wordcloud.png}
        \caption{Intelligent Design Assistants}
        \label{fig:sub-design}
    \end{subfigure}
    \caption{Word clouds visualizing key terms in two representative software request clusters}
    \label{fig:requests_wordclouds}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{analysis/plots/role_vs_autonomous_software_request_cluster_heatmap.png}
    \caption{Relationship between Professional Role and Autonomous Software Request Clusters}
    \label{fig:role_requests_heatmap}
\end{figure}

\subsection{Key Insights from Clustering Analysis}
The K-means clustering analysis revealed several important insights that complement our earlier findings:

\begin{enumerate}
    \item \textbf{Role-Specific Priorities:} Each professional role demonstrated distinct clusters of priorities, concerns, and requests that align with their specific workflows and contexts.
    
    \item \textbf{Trust-Based Narratives:} Trust level in AI strongly influenced the narrative frames participants used when discussing AI, with high-trust users emphasizing transformation and innovation, while low-trust users focused on preservation and tradition.
    
    \item \textbf{Experience-Based Patterns:} Coding experience correlated with distinct clusters across all three open-ended questions, with novices favoring educational support, mid-career professionals seeking balanced integration, and highly experienced developers showing polarized preferences.
    
    \item \textbf{Context-Specific Concerns:} Each cluster revealed context-specific concerns and priorities that might be missed in broader categorical analysis, such as the distinction between security concerns for client-facing freelancers versus academic integrity concerns for researchers.
\end{enumerate}
